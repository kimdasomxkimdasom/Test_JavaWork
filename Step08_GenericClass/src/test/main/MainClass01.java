...
public class MainClass01 {
	public static void main(String[] args) {
		...
		FruitBox<Apple> box1=new FruitBox<Apple>(); //FruitBox객체 생성
		FruitBox<Banana> box2=new FruitBox<Banana>();
		FruitBox<Orange> box3=new FruitBox<Orange>();
		
		//setItem( ) 메소드에 전달해야하는 type이
		//Generic Class에 의해서 정해진다.
		box1.setItem(new Apple()); //apple객체 생성
		box2.setItem(new Banana());
		box3.setItem(new Orange());
		
		//getItem( ) 메소드가 리턴해주는 type이
		//Generic Class에 의해서 정해진다.
